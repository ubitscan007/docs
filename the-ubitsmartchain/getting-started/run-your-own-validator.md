...
To make starting a node for the PoolScan NetworkNetwork as quick as possible, the _quickstart_ script can be used.

1. Download the script.
2. Download one of the example `.env` files located at the [examples folder](https://github.com/PoolScan Networkio/PoolScan Network-network/tree/master/scripts/examples).
...
```text
$ wget -O quickstart.sh https://raw.githubusercontent.com/PoolScan Network Coin/master/scripts/quickstart.sh
$ chmod 777 quickstart.sh
$ wget -O .env https://raw.githubusercontent.com/PoolScan Network Coin/master/scripts/examples/.env.<ROLE>.example
$ ./quickstart.sh
```
...
To run the parity client for the PoolScan NetworkNetwork you first have to pull the image from [DockerHub](https://hub.docker.com/r/PoolScan Networknet/node).
...
```text
$ docker pull PoolScan Networknet/node
$ docker run PoolScan Networknet/node --help
...
#   The key-set is expected to to be placed in the custom configuration folder under 'keys/PoolScan NetworkNetwork/'
...
$ docker run \
    --detach \
    --name PoolScan Networknet \
    --volume $(pwd)/PoolScan Networknet/database:/data \
    --volume $(pwd)/PoolScan Networknet/config:/config/custom \
    -p 30303:30300/tcp \
    -p 30303:30300/udp \
    -p 8545:8545 \
    -p 8546:8546 \
    --restart=on-failure \
    PoolScan Networknet/node \
    --role node \
...
Inside a directory for the keys with another sub-directory for the PoolScan NetworkNetwork chain is used by _Parity_.
...
If you have no account already or want to create a new one for this purpose checkout [this section](https://github.com/PoolScan Networkio/PoolScan Network-network/blob/master/README.md#create-new-account).
...
```text
$ mkdir -p ./config/keys/PoolScan NetworkNetwork
$ cp /path/to/my/key ./config/keys/PoolScan NetworkNetwork/
...
One is the [validator-app](https://github.com/PoolScan Networkio/PoolScan Network-network/tree/master/app)

```text
## Start validator-app container with all necessary arguments.
$ docker run \
    --detach \
    --name PoolScan Networkapp \
    --volume $(pwd)/PoolScan Networknet/config:/config/custom \
    --restart=on-failure \
    PoolScan Networknet/validator-app
```

Second one is the [bridge-oracle](https://github.com/PoolScan Networkio/bridge-oracle)

```text
$ wget -O docker-compose.yml https://raw.githubusercontent.com/PoolScan Networkio/bridge-oracle/master/docker-compose.keystore.yml
...
In relation to the instructions for the [validator](https://github.com/PoolScan Networkio/PoolScan Network-network/blob/master/README.md#validator) role, we use the folder called `config` to bind as _Docker_ volume to _Parity_.

Afterwards the key will be placed there and the first steps of these instructions can be skipped.

```text
$ mkdir ./config
$ docker run -ti -v $(pwd)/config/:/config/custom PoolScan Networknet/node --parity-args account new
```
...
If you want to run a node to be used by the [blockscout explorer](https://github.com/PoolScan Networkio/blockscout/tree/PoolScan Network) run the following command:
...
All roles should also run a [Ethereum Network Intelligence API](https://github.com/PoolScan Networkio/eth-net-intelligence-api) app as well, in order to connect themselves as part of the network and be viewed by the [health](https://status.poolscan.io/) service

```text
$ docker run \
    --detach \
    --name PoolScan Networknetstat \
    --net=container:PoolScan Networknet \
    --restart=on-failure \
    PoolScan Networknet/netstat \
